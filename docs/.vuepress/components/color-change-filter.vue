<template>
  <div>
    ...
    <p id="loss-detail" class="filter-loss" v-show="filter">
...
export default {
  ...
  methods: {
    ...
    change() {
      let self = this;
      let startDom = this.$refs.start.$el;
      let endDom = this.$refs.end.$el;
      ...
      canvas.height = 10;
      // 将输入的颜色转换为rgb格式
      let startRgbColor = window.getComputedStyle(startDom).color;
      let endRgbColor = window.getComputedStyle(endDom).color;
      let startRgbColorList = startRgbColor
        .replace(/\s|[A-Za-z\(\)]/g, "")
        .split(",");
      let endRgbColorList = endRgbColor
        .replace(/\s|[A-Za-z\(\)]/g, "")
        .split(",");
      // 目标颜色对象
      let tagetColor = new self.Color(
        endRgbColorList[0],
        endRgbColorList[1],
        endRgbColorList[2]
      );
      let solver = new self.Solver(tagetColor);
      ...
      img.onload = function() {
        ...
        let imageData = context.getImageData(5, 5, 1, 1);
