...
虽然包装对象看上去和原来的值一模一样，但他们的类型已经变为 object 了！所以，包装对象和原始值用===比较会返回 false：
...
- min()和 max()方法
  ...
  //或者 var max = Math.max.apply(null, values);
  ```
...
当调用静态或原型方法时，没有值为“this”的对象(或“this”作为布尔，字符串，数字，未定义或null) ，那么“this”值在被调用的函数内部将为**undefined**。不会发生自动装箱。即使我们以非严格模式编写代码，它的行为也是一样的，因为所有的函数、方法、构造函数、getters或setters都在严格模式下执行。因此如果我们没有指定this的值，this值将为**undefined**。
