...
- 浏览器的Event loop是按照[html标准](https://html.spec.whatwg.org/multipage/webappapis.html#event-loops)定义来实现，具体的实现留给各浏览器厂商
...
也就是说，Vue绑定的数据发生变化时，页面视图不会立即重新更新，需要等到当前任务执行完毕时进行更新。
